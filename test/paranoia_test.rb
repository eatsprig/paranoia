...
def setup!
  connect!
  {
    ...
    'non_paranoid_employers' => 'name VARCHAR(32)',
...
class ParanoiaTest < test_framework
  ...
  def test_validates_uniqueness_only_checks_non_deleted_records_at_save
    a = Employer.create!(name: "A")
    b = Employer.new(name: "A")
    a.destroy
    assert b.valid?
  ...
  def test_validates_uniqueness_for_non_paranoid
    NonParanoidEmployer.create!(name: "A")
    duplicate = NonParanoidEmployer.new(name: "A")
...
class NonParanoidEmployer < ActiveRecord::Base
  validates_uniqueness_of :name
  has_many :jobs
  has_many :employees, :through => :jobs
end
